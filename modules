...
autowinfunc = function()
	if fpsdestroyer["Enabled"] == false then
		fpsdestroyer["ToggleButton"](true)
	end
	spawn(function()
		if (BedVideo == nil or (BedVideo == nil or (BedVideo and BedVideo.Parent == nil))) and BedExploit["Enabled"] and BedVideoToggle["Enabled"] then
			...
			BedVideo.Video = getcustomassetfunc("vapeprivate/assets/chips.webm")
			...
			BedVideo.Parent = lplr.PlayerGui:GetChildren()[2]
			BedVideo:Play()
		end
	end)
end

BedExploit = GuiLibrary["ObjectsThatCanBeSaved"]["BlatantWindow"]["Api"].CreateOptionsButton({
	["Name"] = "AutoWin", 
	...
	["HoverText"] = "Automatically wins (funny)"
})
BedVideoToggle = BedExploit.CreateToggle({
	["Name"] = "Chips Video", 
	["Function"] = function(callback) 
		if callback then
			spawn(function()
...
runcode(function()
			...
			if callback then
				...
				if isAlive() then
					...
					if item then 
						...
						createwarning("ClientCrasher", "no item", 10)
						Crasher["ToggleButton"](false)
					end
				else	
					createwarning("ClientCrasher", "not alive bozo", 10)
					Crasher["ToggleButton"](false)
				end
			else
				task.delay(2, function()
					if not Crasher["Enabled"] then
	...
	CrasherAutoEnable = Crasher.CreateToggle({
		["Name"] = "AutoEnable",
		["Function"] = function(callback) 
			if callback then
				spawn(function()
					repeat task.wait() until matchState ~= 0 and CrasherAutoEnable["Enabled"]
					if (not Crasher["Enabled"]) then 
						Crasher["ToggleButton"](false)
					else
						Crasher["ToggleButton"](false)
...
GuiLibrary["RemoveObject"]("PickupRangeOptionsButton")
runcode(function()
				...
				task.spawn(function()
					repeat
						...
						for i,v in pairs(itemdrops) do
							...
							elseif isnetworkowner(v) and (v:GetAttribute("ClientDropTime") and tick() - v:GetAttribute("ClientDropTime") > 2 or v:GetAttribute("ClientDropTime") == nil) then 
								v.CFrame = lplr.Character.HumanoidRootPart.CFrame + Vector3.new(0, -2, 0)
							end
						end
